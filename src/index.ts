...
export class Topbar {
  ...
  private barColors: Record<string, string>;
  ...
  progress(to?: number | string) {
    if (typeof to === "undefined") return this.currentProgress;
    if (typeof to === "string") {
  ...
  private loopShow() {
    this.progressTimerId = window.requestAnimationFrame(
      this.loopShow.bind(this),
    );
    this.progress(
...
let topbar: Topbar;
export function config(options: Partial<TopBarOptions>) {
  topbar = new Topbar(options);
  return topbar;
}

export function show(delay?: number) {
  if (!topbar) {
    topbar = new Topbar();
  }
  topbar.show(delay);
...
export function progress(to?: number | string) {
  if (!topbar) {
    topbar = new Topbar();
  }
